...
/*
This example removes the service layer to show manual construction of commands
and using the components by themselves. It does use the HubStore (SQLITE).

At it's most verbose configuration, a complete HubJob, is 100 lines (plus my comments).
Is that alot? In my opinion, probably. But this example shows the user handling
the execution layer by themselves. No "LocalExecutor" or "HubCommandService".

Don't worry, you're going to bypass all this by just using the Hub CLI tool. :D
*/
...
func GetHackerNews() {
	...
	ctx, cancel := getDefaultCtx() //(runutils.go) Background ctx w/ 10 second timeout
